    ...
    <div className="min-h-screen bg-navy bg-glow-radial pb-24 md:pb-0">
      ...
      <PageTransition>
        <main className="mx-auto max-w-3xl px-4 py-6 sm:px-6 sm:py-8">
          ...
          <h1 className="font-display text-2xl font-bold text-slate-100">Attendance</h1>
          ...
          {loading ? (
            ...
            <>
              <div className="mt-6 grid grid-cols-3 gap-3">
                ...
                <Card className="text-center">
                  <p className="font-mono text-2xl font-bold text-emerald-300">{summary?.present_or_late ?? 0}</p>
                  <p className="mt-1 text-[11px] uppercase tracking-wide text-slate-400">Attended</p>
                </Card>
                <Card className="text-center">
                  <p className="font-mono text-2xl font-bold text-rose-300">
                ...
                {records.length === 0 ? (
                  ...
                  records.map((r, i) => (
                    <Card key={r.id} className="flex items-center justify-between animate-fade-in-up" style={{ animationDelay: `${i * 30}ms` }}>
                      <div>
                        <p className="text-sm font-medium text-slate-100">Meeting #{r.meeting_id}</p>
